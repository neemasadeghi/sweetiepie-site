...
export async function getProjects(): Promise<Project[] | null> {
  ...
  try {
    raw = await client.fetch(
      `*[_type == "project"] | order(orderRank asc) {
        ...
        category,
        still,
        "muxPlaybackId": previewMux.asset->playbackId,
  ...
  return raw.map((p: any) => ({
    ...
    shareImageUrl: urlFor(p.still).width(1200).height(630).fit("crop").quality(85).url(),
...
export async function getProjectBySlug(slug: string): Promise<Project | null> {
  ...
  return {
    ...
    shareImageUrl: urlFor(raw.still).width(1200).height(630).fit("crop").quality(85).url(),
